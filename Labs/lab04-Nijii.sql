...
where o.orderdate = (select max(orderdate) from sales.orders o)
...
where o.custid = (select top (1) custid from sales.orders group by custid order by count(orderid) desc);
